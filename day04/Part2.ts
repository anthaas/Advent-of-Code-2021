...
for (const element of draws) {
    ...
    if (bingoBoard.length != 0 && !playLastBoard) {
        boards = boards.filter(item => !isBingo(item))
        if (boards.length == 1) {
            //last board still need to play BINGO!
            playLastBoard = true;
            continue;
        }
    }
    if (playLastBoard && bingoBoard.length != 0) {
